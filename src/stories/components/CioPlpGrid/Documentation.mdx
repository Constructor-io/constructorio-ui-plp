...
<Meta of={CioPlpGridStories} name="Documentation" />
...
### Using RenderProps
If you prefer to handle the rendering of the results yourself, you may pass a Render Props function to the component.
...
function myApp() {
  ...
  return (
    <CioPlp apiKey={MY_API_KEY}>
      <CioPlpGrid />
        {({data, status, refetch}) => data?.response?.results?.map((item) => <li>{item.itemName}</li>)}
      </CioPlpGrid>
    </CioPlp>
  )
}
