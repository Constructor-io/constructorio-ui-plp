...
| perFacetConfigs        | `Record<string, FacetConfig>`                     | Per-facet config overrides (e.g. `{ color: { renderVisual: true, collapsed: true } }`) |
...
| renderCollapsed        | `boolean`              | When `true`, all filter groups render collapsed by default. When `false`, all render expanded (overrides metadata). Per-facet overrides via `perFacetConfigs` take precedence. |
