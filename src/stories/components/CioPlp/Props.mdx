...
| perFacetConfigs        | `Record<string, FacetConfig>`                     | Per-facet config overrides (e.g. `{ color: { isVisualFacet: true, isCollapsed: true, checkboxPosition: 'none' } }`) |
...
By default the library performs **SPA navigation**: it updates the URL via
`history.pushState` (no full page reload) and dispatches a `popstate` event so the PLP
re-renders and refetches. To opt into a full page reload instead, see
[SPA Navigation](./?path=/docs/usage-examples-spa-navigation--documentation#opting-out-hard-refresh-on-every-url-change).
