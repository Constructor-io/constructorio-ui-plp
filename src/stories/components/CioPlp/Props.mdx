...
| onShowMoreSwatches | `(event: React.MouseEvent, item: Item, selectedSwatch: SwatchItem \| undefined, hiddenSwatches: SwatchItem[], setUrl: UrlHelpers['setUrl']) => void` | Callback for the "View more" button on swatches. When not provided, clicking "View more" navigates to `selectedSwatch.url \|\| item.url` via `setUrl`. (📦 v1.9.1 required) |
