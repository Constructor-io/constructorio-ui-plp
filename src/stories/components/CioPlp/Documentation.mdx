...
function MyComponent() {
  return (
    <CioPlp>
      {({ cioClient, cioClientOptions, setCioClientOptions, formatters, itemFieldGetters, urlHelpers }) => <div />}
    </CioPlp>
  );
}
```
...
function ChildOfCioPlp(params) {
  const { cioClient, cioClientOptions, setCioClientOptions, formatters, itemFieldGetters, urlHelpers } =
    useCioPlpContext();
