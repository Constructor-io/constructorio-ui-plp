...
To hide specific groups, you can pass [groupOptions.isHiddenGroupFn](./?path=/docs/components-groups--props) prop at the top CioPlp-level.
...
const groupConfigs = {
  isHiddenGroupFn: (group) => { 
    if (group.data.cio_plp_hidden === true) { // Customize this conditional!
      return true;
    }
