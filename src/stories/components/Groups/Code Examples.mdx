...
Use `componentOverrides` on the `CioPlp` provider to replace default sub-components within `Groups`. Each override slot accepts either a static `ReactNode` or a render function that receives `GroupsRenderProps` with full state access (`groups`, `breadcrumbs`, `currentPage`, `isCollapsed`, `toggleIsCollapsed`, `optionsToRender`, `isShowAll`, `setIsShowAll`, `selectedGroupId`, `onOptionSelect`, `goToGroupFilter`).
