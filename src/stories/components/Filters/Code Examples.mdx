...
1. **`perFacetConfigs[name].collapsed`** (per-facet config) — highest priority
      ...
      <CioPlpGrid>
        {(props) => (
          <Filters
            facets={props.filters.facets}
            perFacetConfigs={{
              color: { renderVisual: true },
              'Base Color': { renderVisual: true },
            }}
          />
        )}
      </CioPlpGrid>
...
1. `perFacetConfigs[name].renderVisual` — explicit per-facet override
