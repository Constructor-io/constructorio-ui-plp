...
<div
  ...
  <p style={{ margin: '0 0 16px' }}>
    <strong>SPA navigation is now the default.</strong> The default `setUrl` is now based on a{' '}
    <code>history.pushState</code>. Interactions with <code>CioPlp</code> like facet selection, sort
    changes, pagination, and Back/Forward buttons update the URL{' '}
    <strong>without a full page reload</strong>.
  </p>
  <p style={{ margin: 0 }}>
    If you added <code>urlHelpers</code> overrides only to get SPA navigation, you may now remove
    them.
  </p>
</div>
...
export function PlpPage() {
  ...
  return <CioPlp key={href} apiKey='YOUR_KEY' urlHelpers={urlHelpers} />;
}
```
