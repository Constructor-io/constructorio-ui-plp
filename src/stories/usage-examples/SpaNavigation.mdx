...
- **Remount `<CioPlp />` on URL changes** (recommended) by keying it off an incrementing `plpKey` counter (example below).
...
export function PlpPage() {
  ...
  const [plpKey, setPlpKey] = React.useState(1);
...
- The PLP reads request state from the URL, but fetch hooks currently run on mount. In SPA routing, URL updates do not necessarily remount components, so updating a `plpKey` counter and passing it as the `<CioPlp />` `key` is a simple way to ensure it refetches.
