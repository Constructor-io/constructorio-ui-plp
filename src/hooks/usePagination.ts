...
const usePagination: UsePagination = (searchResponse, searchRequest, windowSize = 5) => {
  const [totalPages, setTotalPages] = useState(0);
  const [currentPage, setCurrentPage] = useState(1);

  // Calculate total number of pages
  useEffect(() => {
    if (searchResponse?.totalNumResults && searchRequest?.num_results_per_page) {
      setTotalPages(Math.ceil(searchResponse.totalNumResults / searchRequest.num_results_per_page));
    }
  }, [searchResponse?.totalNumResults, searchRequest?.num_results_per_page]);
